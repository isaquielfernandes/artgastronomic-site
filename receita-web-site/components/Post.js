...
const  Post = ({ posts, loading }) => {
    ...
    return (
        <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 clearfix shadow-sm" itemtype ="http://schema.org/Receita">
            { posts.map((p) => (
                    ...
                    <div id={p.fields.categoria} className="card rounded-0 h-100 img-container" >
                        ...
                        <div className="card-body p-2">
                            <h5 className="font-weight-lighter text-truncate my-3">{p.fields.nome}</h5>
                             <Link className="" href="/post/[id]" as={`/post/${p.sys.id}`}>
                                <a className="btn btn-outline-warning mb-3">Ver Receita <i className="fa fa-cutlery" aria-hidden="true"></i></a>
                             </Link>
                        </div>
                    </div>
                </article>
