
const Frases = () => (

    <div className="col-12 col-lg-5 shadow-sm card rounded my-2 mx-auto">
        <div className="card-body message-body text-center py-5">
            <img src="img/logo/logotipo.jpg" className="img-fluid img-thumbnail img-logo mb-2" width={120} height={120} alt="logotipo chefsassyfnandes" />
            <h4 className="card-title text-center"><b>Sassy Fernandes</b></h4>
            <h6 className="card-subtitle mb-2 text-muted" />
            <p className="card-text text-center text-secondary">"COZINHAR É COMO A VIDA.
                        CASE COM UMA PESSOA QUE SAIBA COZINHAR...
                        O AMOR ACABA...A FOME NÃO."
            </p>
        </div>
    </div>
);

export default Frases;
